...
def train_baseline(model_name, model, train_loader, test_loader, device, epochs, save_path, task, cls_num_list,
    ...
    recorder = MetricsRecorder(
        ...
        threshold=None, start_revision=None, mode="baseline",
    )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, labels, sample_ids) in progress_bar:
            ...
            with torch.no_grad():
                ...
                correct_class_prob = prob[torch.arange(labels.size(0)), labels]
                sample_losses = per_sample_loss(outputs, labels)
                kept_mask = torch.ones_like(labels, dtype=torch.bool)  # baseline never drops samples
                recorder.log_batch(epoch, sample_ids.tolist(), labels, preds, outputs, sample_losses, correct_class_prob, kept_mask)
                if epoch == 0:
    ...
    with torch.no_grad():
        for inputs, labels, sample_ids in tqdm(indexed_train_loader, desc="Final outcomes pass"):
            ...
            final_probs.append(correct_class_prob)
...
def train_baseline_noisy(model_name, model, train_loader, test_loader, device, epochs, save_path, task, cls_num_list):
    model.to(device)
